...
public class ToolBar extends JToolBar {
	...
	public ToolBar (ActionListener buttonActionListener, SearchCallback searchCallback) {
		...
		// Max Latz
		add(createToolBarButton("sort", "sortFeeds",
				"Sort the Feed List", "Sort", false, buttonActionListener));
		...
		searchField.getDocument().addDocumentListener(new DocumentListener() {
			 ...
			 * Also die Suche so aufzurufen, mit nem Backround worker/thread 
			 * fürt halt dazu das man wenn man schnell genug tippt
			 * mehrere Suchen quasi gleichzeitig losschickt. 
			 * Die Suche die dann als letztes fertig wird, wird angezeigt. 
			 * Is halt nicht optimal, weil das nicht immer das ist was man suchen wollte.
			 * 
			 * Viele große Feeds und eine Suche wie "*e*" helfen beim reproduzieren. (Geht auch in der Demo)
			 * 
			 * Ein mölicher Fix wäre ein Delay bei der Abfrage das search Field 
			 * und das verhindern von mehrfach Aufrufe.
			 * -> siehe startSearch()
			 */
			...
			*/
			@Override
			public void removeUpdate(DocumentEvent e) { startSearch(searchCallback); }
			@Override
			public void insertUpdate(DocumentEvent e) { startSearch(searchCallback); }
			@Override
			public void changedUpdate(DocumentEvent e) { startSearch(searchCallback); }
		});
	...
	/**
	 * quick and dirty delay for searchField.getText() and start of search()
	 * wont start multiple instances of the same search
	 * @author Florian Hensel
	 * @param searchCallback
	 */
	private void startSearch(SearchCallback searchCallback){
		if(!searchExists) {
			searchExists = true;
